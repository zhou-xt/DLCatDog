...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(3, 6, 5), nn.BatchNorm2d(6),nn.ReLU(), nn.MaxPool2d(2, 2))
        self.conv2 = nn.Sequential(nn.Conv2d(6, 16, 5), nn.BatchNorm2d(16), nn.ReLU(), nn.MaxPool2d(2, 2))
        self.conv3 = nn.Sequential(nn.Conv2d(16, 16, 5), nn.BatchNorm2d(16), nn.ReLU())
        self.incenption = InceptionModel()
        self.fc1 = nn.Sequential(nn.Linear(64 * 25 * 25, 1024), nn.BatchNorm1d(1024),nn.Dropout(p=0.5),nn.ReLU())
        self.fc2 = nn.Sequential(nn.Linear(1024, 512), nn.BatchNorm1d(512),nn.Dropout(p=0.5),nn.ReLU())
...
def train():
    ...
    print('train:', epoch + 1, 'epoch', ' lr:',lr,' loss:', '%.4f ' % (running_loss / train_total), ' accrary:',
          int(train_correct),
